...
netflix_dataset['genre'] = netflix_dataset['listed_in'].fillna('')
netflix_dataset['category'] = netflix_dataset['rating'].fillna('')
...
def fetch_movie_info(title):
    ...
    if response.status_code == 200:
        ...
        if movie_data['Response'] == 'True':
            ...
            category = movie_data.get('Type', '')
            poster = movie_data.get('Poster', '')
            return description, genre, category, poster
...
def show_recommendation(title, diversity_factor=20):
    description, genre, category, poster = fetch_movie_info(title)
    if description:
        # Add the fetched description to the dataframe temporarily
        temp_row = pd.DataFrame({'title': [title], 'description': [
                                description], 'genre': [genre], 'category': [category]})
        ...
        # Calculate the cosine similarity matrix for the TF-IDF matrix
        cos_sim_temp = cosine_similarity(tfidf_matrix_temp, tfidf_matrix_temp)
